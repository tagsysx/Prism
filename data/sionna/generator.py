...
DEFAULT_CONFIG = {
    ...
    'max_depth': 5,
    'num_samples': int(1e5),
    'use_empty_scene': False,  # True for free-space, False for campus scene
...
class RayTracingDataGenerator:
    ...
    def _setup_device(self):
        ...
        try:
            ...
            if self.gpu_id is not None:
                ...
                if self.gpu_id < len(gpu_devices):
                    tf.config.experimental.set_visible_devices(gpu_devices[self.gpu_id], 'GPU')
                    logger.info(f"Using GPU {self.gpu_id}: {gpu_devices[self.gpu_id].name}")
                else:
                    logger.warning(f"GPU {self.gpu_id} not available. Available GPUs: {len(gpu_devices)}")
                    logger.info("Falling back to default device")
            else:
                # Use default device
                gpu_devices = tf.config.experimental.list_physical_devices('GPU')
                if gpu_devices:
                    logger.info(f"Using default GPU: {gpu_devices[0].name}")
                else:
                    logger.info("No GPU available, using CPU")
                    
        except ImportError:
            logger.warning("TensorFlow not available, cannot set GPU device")
        except Exception as e:
            logger.warning(f"Failed to setup GPU device: {e}")
            logger.info("Continuing with default device")
        
    def load_empty_scene(self):
        """Create a free-space simulation using campus scene with minimal ray tracing."""
        import sionna
        import os
        scene_path = os.path.join(os.path.dirname(sionna.__file__), "rt", "scenes", "munich", "munich.xml")
        
        logger.info("Loading campus scene for free-space simulation (minimal ray tracing)")
        ...
        # Configure antennas for free-space simulation
        self._configure_antennas()
        
        # Override ray tracing parameters for free-space simulation
        self.config['max_depth'] = 1  # Only direct path (no reflections)
        self.config['num_samples'] = int(1e3)  # Reduced samples for faster computation
        
        logger.info("Free-space scene configured successfully (direct path only)")
    ...
    def load_campus_scene(self):
        ...
        # Configure antennas for campus scene
        self._configure_antennas()
        
        logger.info("Campus scene loaded successfully")
    
    def _configure_antennas(self):
        """Configure antenna arrays for both empty and campus scenes."""
    ...
    def run_simulation(self, output_path):
        ...
        if self.config.get('use_empty_scene', False):
...
def parse_arguments():
    ...
    parser.add_argument('--gpu', type=int, default=None, help='GPU device ID to use (e.g., 0, 1, 2). If not specified, uses default device.')
    parser.add_argument('--empty_scene', action='store_true', help='Use empty scene for free-space propagation instead of campus scene')
...
def main():
    ...
    # Create output path based on number of positions
    if args.output_path == './sionna_results':  # If using default path
        output_path = f'./results/P{args.num}'
    else:
    ...
    config.update({
        ...
        'use_empty_scene': args.empty_scene,
    ...
    print(f"场景类型: {'自由空间 (空场景)' if config['use_empty_scene'] else '校园环境 (慕尼黑场景)'}")
    print(f"Output path: {config['output_path']}")
    if args.gpu is not None:
        print(f"GPU device: {args.gpu}")
    else:
        print("GPU device: Default")
    print()
    ...
    # Run simulation
    generator = RayTracingDataGenerator(config, gpu_id=args.gpu)
