#!/usr/bin/env python3
"""
PolyU WiFi Training Script for Prism Neural Network

This script provides a training pipeline specifically adapted for PolyU WiFi data,
handling the unique data format where bs_position has only one dimension (SSID)
and needs to be padded to 3D coordinates.

Features:
- Adapted data loading for PolyU WiFi format
- BS position padding (1D SSID -> 3D coordinates with Y,Z=0)
...
class PolyUTrainer:
    """
    PolyU WiFi training pipeline for Prism neural network system.
    
    Adapted training implementation specifically for PolyU WiFi data format
    with BS position padding and proper data structure handling.
    """
    ...
    def __init__(self, config_path: str, resume_from: Optional[str] = None):
        ...
        print("🔧 Loading PolyU configuration...")
        try:
        ...
        self.logger.info("🚀 PolyUTrainer initialized successfully")
    ...
    def _create_model(self):
        ...
        self.logger.info("🔧 Creating PrismNetwork for PolyU data...")
        ...
        self.logger.info(f"✅ PrismNetwork created for PolyU:")
    ...
    def _load_polyu_data(self):
        """Load and prepare PolyU WiFi training data with BS position padding."""
        self.logger.info("🔧 Loading PolyU WiFi training data...")
        
        # Construct dataset path for PolyU data
        dataset_path = "data/polyu/data.h5"
        
        if not os.path.exists(dataset_path):
            raise FileNotFoundError(f"PolyU dataset not found: {dataset_path}")
        
        # Load PolyU data
        with h5py.File(dataset_path, 'r') as f:
            # Extract data from PolyU HDF5 structure
            ue_positions = torch.from_numpy(f['data/ue_positions'][:]).float()
            bs_positions_raw = torch.from_numpy(f['data/bs_positions'][:]).float()  # Shape: (913, 1) - SSID values
            channel_responses = torch.from_numpy(f['data/csi'][:]).cfloat()  # Shape: (913, 8, 1, 64)
            
            self.logger.info(f"📊 Raw PolyU data shapes:")
            self.logger.info(f"   UE positions: {ue_positions.shape}")
            self.logger.info(f"   BS positions (raw): {bs_positions_raw.shape}")
            self.logger.info(f"   CSI data: {channel_responses.shape}")
            
            # Convert CSI data from [samples, ue_antennas, bs_antennas, subcarriers] to [samples, bs_antennas, subcarriers, ue_antennas]
            # PolyU format: (913, 8, 1, 64) -> (913, 1, 64, 8)
            csi_data = channel_responses.permute(0, 2, 3, 1)  # [samples, bs_antennas, subcarriers, ue_antennas]
            
            # Handle BS positions: Convert 1D SSID values to 3D coordinates
            num_samples = ue_positions.shape[0]
            self.logger.info(f"🔧 Converting BS positions from 1D SSID to 3D coordinates...")
            
            # PolyU BS positions are SSID values (0.0, 1.0, 2.0), need to convert to 3D coordinates
            # Strategy: Use SSID as X coordinate, set Y=0, Z=0
            bs_positions = torch.zeros(num_samples, 3)
            bs_positions[:, 0] = bs_positions_raw.squeeze()  # X = SSID value
            bs_positions[:, 1] = 0.0  # Y = 0 (padded)
            bs_positions[:, 2] = 0.0  # Z = 0 (padded)
            
            self.logger.info(f"✅ BS position conversion completed:")
            self.logger.info(f"   Original SSID range: [{bs_positions_raw.min():.1f}, {bs_positions_raw.max():.1f}]")
            self.logger.info(f"   Converted to 3D coordinates: {bs_positions.shape}")
            ...
            num_bs_antennas = csi_data.shape[1]  # Should be 1 for PolyU
        ...
        self.logger.info(f"✅ PolyU data loaded successfully:")
        self.logger.info(f"   UE positions: {ue_positions.shape}")
        self.logger.info(f"   BS positions (converted): {bs_positions.shape}")
        self.logger.info(f"   CSI data: {csi_data.shape} [samples, bs_antennas, subcarriers]")
    ...
    def train(self):
        """Main training loop."""
        self.logger.info("🚀 Starting PolyU WiFi Prism neural network training")
        ...
        self._load_polyu_data()  # Use PolyU-specific data loading
        ...
        try:
            ...
            self.logger.info("✅ PolyU WiFi training pipeline completed successfully")


def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Train Prism Neural Network on PolyU WiFi Data')
